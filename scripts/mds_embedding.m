...
for hemi_idx = 1:length(hemis)
	...
	labels = repelem(...
		[{'Blind'}, {'Sighted Video'}, {'Sighted Audio'}],...
		[size(blind_data, 2), size(sighted_video_data, 2), size(sighted_audio_data, 2)]...
	)';

	number_labels = [...
		1:size(blind_data, 2),...
		1:size(sighted_video_data, 2),...
		1:size(sighted_audio_data)...
	];
